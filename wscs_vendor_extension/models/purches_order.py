...
class PurchaseOrder(models.Model):
    ...
    def create(self, vals):
        ...
        if partner_id:
            ...
            if  partner.status_id.prevent_po_creation in ['no'] and (partner.status_grade2 or partner.status_grade1):
                status_record = status_record = self.env['vendor.status'].search([('is_active', '=', True)])
                partner.status_id = status_record
            elif partner.status_id.prevent_po_creation in ['alert']:
                status_record = self.env['vendor.status'].search([('is_active', '=', True)])
                partner.status_id = status_record
